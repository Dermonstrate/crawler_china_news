...
def get_page(url):
    ...
    Cookie_list = ['ll="108288"; bid=yWvGAVL6Q8c; _pk_id.100001.8cb4=9ef052a9c4643216.1636442574.1.1636442574.1636442574.; _pk_ses.100001.8cb4=*; __utma=30149280.218475865.1636442585.1636442585.1636442585.1; __utmc=30149280; __utmz=30149280.1636442585.1.1.utmcsr=(direct)|utmccn=(direct)|utmcmd=(none); __utmt=1; __utmb=30149280.1.10.1636442585; dbcl2="147748543:Qa+y+Ngo/88"; ck=Kola',
                   ...
                   'Hm_lvt_0da10fbf73cda14a786cd75b91f6beab=1657846226; Hm_lpvt_0da10fbf73cda14a786cd75b91f6beab=1657846249']
    random_num_cookie = random.randint(0,2)
    random_num_agent = random.randint(0,4)

    headers = {
        'Cookie': Cookie_list[3],
        'User-Agent': user_agent_list[5]
    }
...
def get_content(html, line):
    #通过首页获取全部详情页的url
    doc = pq(html)
    items =doc("#con .div980 .con_left .content .left_zw").items
    for item in items():
        ...
        if content=="None":
            continue
        file = open('D://internalship//Deloitte_consulting//Python_crawler_data//article//'+ line +'.txt', 'a', encoding='utf-8')
        file.write('\n'.join([str(content)]))
        file.close()

f = open('D://internalship//Deloitte_consulting//Python_crawler_data//url_data.txt')
for i in range(0, 6890):#逐行读取txt文件中的url
    line = f.readline()
    line = line.replace("\n", "")
    if i<852:
        continue
    html = get_page(line)
    get_content(html, str(i))
f.close()
